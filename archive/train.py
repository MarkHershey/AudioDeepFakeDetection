...
import numpy as np
import torch
from dataset import WavDataset, get_dataset
...
def train(args, model, train_loader, val_loader, patience, loss_func, optimizer):
    ...
    while num_epoch_with_no_progress < patience:
        model.train()
        total_loss = 0
        for i, (x, rate, y) in enumerate(
            tqdm(train_loader, desc="Training epoch" + str(current_epoch) + ":")
        ):
            ...
            if i % 500 == 0:
                print(
                    "epoch %d instance %d loss: %f", current_epoch, i, total_loss / 500
                )
                LOGGER.info(
                    "epoch %d instance %d loss: %f"
                    % (current_epoch, i, total_loss / 500)
                )
                total_loss = 0
...
def main():
    ...
    parser.add_argument(
        "--train_val_test_split", nargs="+", type=int, default=[0.6, 0.2, 0.2]
    )
    parser.add_argument(
        "--seq_length",
        type=int,
        default=10,
        help="shard video clips into how many peices and feed into RNN",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="will stop training if val accuracy does not increase for --patience epochs",
    )
    ...
    parser.add_argument(
        "--down_sample_rate",
        help="sampel a subset to finetune network parameters",
        default=1.0,
        type=float,
    )
    parser.add_argument(
    ...
    train_dataset, test_dataset = get_dataset(
        ...
        experiment_1=args.experiment_1,
        experiment_2=args.experiment_2,
    )
