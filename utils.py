...
def get_waveform(filepath: str, resample_rate: int = 16_000, normalize=True) -> Tuple[torch.Tensor, int]:
    """
    ...
    return torchaudio.sox_effects.apply_effects_file(filepath, effects=effects, normalize=normalize)
...
def trim_or_pad_time(data: torch.Tensor, target_time: int) -> torch.Tensor:
    ...
    if data.shape[-1] > target_time:
        ...
        waveform_len = data.shape[-1]
        num_repeats = int( target_time/ waveform_len)+1
        padded_waveform = torch.tile(data, (1, num_repeats))[
            :, :target_time][0]
